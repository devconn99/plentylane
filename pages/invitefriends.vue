        ...
        <div class="authorization-box">
          <div class="authorization-box-position position-relative">
            ...
            <div v-if="alreadyRegisteredUser">
              <h2>Are you sure you want to accept invitation?</h2>
              <b-button @click="onSubmit" class="btnGreen btnBigSize btn50 text-uppercase hover-slide-left">
                <span>Accept</span>
              </b-button>
            </div>

            <b-form  v-else class="form" @submit.stop.prevent="onSubmit">
              <b-form-group>
                <b-form-input
                  name="fullName"
...
export default {
  ...
  data: () => ({
    ...
    registeredUserMail:"",
    pwdMinLength: process.env.PWD_MIN_LENGTH,
    pwdMaxLength: process.env.PWD_MAX_LENGTH,
    userNameMinLength: process.env.USER_NAME_MIN_LENGTH,
    userNameMaxLength: process.env.USER_NAME_MAX_LENGTH,
    submitted: false,
    form: {
      fullName: '',
      email: '',
    ...
    errorMsg: '',
    loaderOptions: { ...process.env.LOADER_OPTIONS },
    isSubmitting: false
  }),
  validations: {
    form: {
      ...
      username: {
        required,
        minLength: minLength(process.env.USER_NAME_MIN_LENGTH),
        maxLength: maxLength(process.env.USER_NAME_MAX_LENGTH)
      },
      password: {
        required,
        minLength: minLength(process.env.PWD_MIN_LENGTH),
        maxLength: maxLength(process.env.PWD_MAX_LENGTH),
        isValidPwd(value) {
  ...
  methods: {
    ...
    isValidPassword (value) {
      if (value === '') return true;
      return new RegExp(`^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z]).{${process.env.PWD_MIN_LENGTH},}$`, 'g').test(value);
    },
    resetError () {
      if (this.submitted) {
      ...
      api.invitations.acceptAlreadyRegisteredInvitation(userData)
        ...
        .catch(err => {
          ...
          if (Array.isArray(err.message) && err.message.length > 0) {
            ...
            this.errorMsg = 'Error accepting invitation.'
          }
          this.isSubmitting = false;
          this.showErrorAlert = true;
        })
