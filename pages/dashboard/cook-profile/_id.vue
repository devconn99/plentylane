  ...
  <div class="dashboard-content" v-if="cookInfo && cookInfo.id">
    <div class="container-fluid cook-profile-page-container">
              ...
              <div class="cook-info mb-3">
                <div class="cook-info-part">
                  <div class="cook-info-benefits">
                    ...
                    -->
                    <UserRating :rating="meanRating"></UserRating>
                  </div>
                </div>
      ...
      <div class="position-relative">
        ...
        <div class="row" v-if="reviewsPage.loaded && canShowReviews">
                    ...
                    <div class="col-sm-4 mb-2 mb-sm-0">
                      <div class="questions-box-author">
                        ...
                        <div class="questions-box-author-title">
                          <span v-if="review.remarks">
                          <span v-for="(remark,i) in review.remarks" :key="i"><span v-if="i>0">,</span>{{remark}} </span>
                          </span>
                          <span v-if="review">{{review.review}}</span>
                          <span>{{review.updatedAt | formatDate}}</span>
                        </div>
                      </div>
                    </div>
                    <div class="col-sm-8">
...
export default {
  name: "CookProfile",
  components: {SvgIcon, ContactCookModal, OfferInfoBlock, ConfirmModal, UserRating},
  data: () => ({
    isLoaded: false,
    cookId: '',
    cookInfo: null,
    ...
    loadPageData (cb) {
      ...
      Promise.all(requests)
        .then(results => {
          ...
          this.loadRating(this.cookId,this.reviewsPage.page)
          this.isLoaded = true;
    ...
    loadRating(cookId,pageSkip){
      api.ratings.getRatingReviewdByUserId(cookId,pageSkip).then(data=>{
        this.$nextTick(() => {
          ...
          if (data.length < 10) {
            this.reviewsPage.isLastPage = true;
          }
        });
      })
    },
  ...
  computed: {
    ...
    meanRating() {
      const totalReviews = this.reviews.reduce((previous,current) => previous + current.rating, 0);
