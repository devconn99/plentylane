...
export default {
  ...
  data: () => ({
    descMaxLength: process.env.MEAL_INFO.DESCRIPTION_MAX_LENGTH,
    quantityMin: process.env.MEAL_INFO.QUANTITY_MIN,
    quantityMax: process.env.MEAL_INFO.QUANTITY_MAX,
    form: {
      name: '',
      description: '',
      quantity: null
  ...
  validations() {
    const form = {
      ...
      description: {
        required,
        maxLength: maxLength(process.env.MEAL_INFO.DESCRIPTION_MAX_LENGTH)
      }
    };
    if (!this.excludeQuantityField) {
      form['quantity'] = {
        required,
        numeric,
        minValue: minValue(process.env.MEAL_INFO.QUANTITY_MIN),
        maxValue: maxValue(process.env.MEAL_INFO.QUANTITY_MAX)
      }
    }
    return { form: form };
  },
