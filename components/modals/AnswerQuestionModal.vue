      ...
      <b-form class="form" @submit.stop.prevent="onSubmit" v-if="!isSubmitted">
        ...
        <template v-if="questionInfo && questionInfo.mealId && questionInfo.meal && questionInfo.meal.name">
          <b-form-group label="About">
            <nuxt-link :to="{ path: '/dashboard/my-meals/' + questionInfo.mealId }" v-slot="{ href }">
...
export default {
  ...
  data: () => ({
    isSubmitted: false,
    answerMaxLength: process.env.TEXT_AREA_MAX_LENGTH,
    form: {
      answer: null
    },
  ...
  validations: {
    form: {
      answer: {
        required,
        maxLength: maxLength(process.env.TEXT_AREA_MAX_LENGTH)
      }
    }
  },
  methods: {
